...
from fastapi.responses import HTMLResponse,RedirectResponse, FileRfrom fastapi import FastAPI, Request, Form, Query, UploadFile, File
from fastapi.templating import Jinja2Templates
...
def submit_code(data: SubmitRequest):
    ...
    if code.startswith("D-"):
        correct = normalize_lines2(DEBUG_CODES[code]["correct_code"])
        user = normalize_lines2(data.user_code)
        print(correct)
        print(user)
        ...
        Dscore = 0 
        for i, (correct_line, user_line) in enumerate(zip(correct, user)):
            is_correct = user_line == correct_line
            print(is_correct)
            print(repr(correct_line))
            print(repr(user_line))
            if is_correct:
                Dscore += 1
